...
fn load_post_args {
    if {~ $REQUEST_METHOD POST && ~ $#post_args 0} {
        for (pair = `` \&\n {cat}) {
            ...
            $n = `` () {echo -n $pair(2) | urldecode | /bin/tr -d $NEWLINE}
        }
        pair = ()
    }
}
...
fn set_cookie name val expiry {
    if {isempty $expiry} {
        echo 'Set-cookie: '^$^name^'='^$^val^'; Path=/; Secure; HttpOnly; SameSite=Lax'
    } {
        echo 'Set-cookie: '^$^name^'='^$^val^'; Expires='$^expiry'; Path=/; Secure; HttpOnly; SameSite=Lax'
    }
}
fn get_cookie name {
    co = `` ';' {echo $HTTP_COOKIE}
